...
$string['cfg_domainhint_desc'] = 'When using the authorization request login flow, pass this value as the "domain_hint" parameter. "domain_hint" is used by some OpenID Connect providers to make the login process easier for users. Check with your provider to see whether they support this parameter.';
...
$string['cfg_loginflow_rocreds_desc'] = 'Using this flow will authorize you with your identity provider, but will not create a session on your identity provider\'s site. For example, if using Office 365 with OpenID Connect, you will be logged in to Moodle but not the Office 365 web applications. Using the authorization request is recommended if you want users to be logged in to both Moodle and your identity provider.  Note that not all identity providers support this flow.';
