...
class forms extends base {
    ...
    public function get_replacement($matched) {
        if (!empty($matched)) {
            $embedhtml = $this->getembedhtml($matched[1].$matched[3].'/'.$matched[4].'/ResponsePage.aspx?id='
                    .$matched[6].'&embed=true');
            return $embedhtml;
        }
        return $matched[0];
    }
    ...
    private function getembedhtml($embedurl) {
        return '<iframe src="'. $embedurl . '" height="768px" width="99%" frameborder= "0" marginwidth= "0" '
                . 'marginheight= "0" style= "border: none; max-width:100%; max-height:100vh"'
