 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignfeedback_onenote_zip_importer {
    ...
    public function extract_files_from_zip($zipfile, $contextid) {
        ...
        core_php_time_limit::raise(ASSIGNFEEDBACK_ONENOTE_MAXFILEUNZIPTIME);
     ...
     *
     * @param assign $assignment - The assignment instance
     * @param assign_feedback_file $fileplugin - The OneNote feedback plugin
    ...
    public function import_zip_files($assignment, $fileplugin) {
            ...
            if ($this->is_valid_filename_for_import($assignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($assignment, $user, $plugin, $filename, $unzippedfile)) {
                    ...
                    if ($oldfile = $fs->get_file($contextid,
                        ...
                        $newfilerecord->component = 'assignfeedback_onenote';
                        $newfilerecord->filearea = ASSIGNFEEDBACK_ONENOTE_FILEAREA;
        ...
        require_once($CFG->dirroot . '/mod/assign/feedback/onenote/renderable.php');
        $importsummary = new assignfeedback_onenote_import_summary($assignment->get_course_module()->id,
                                                            count($userswithnewfeedback),
                                                            $feedbackfilesadded,
                                                            $feedbackfilesupdated);

        $assignrenderer = $assignment->get_renderer();
        $renderer = $PAGE->get_renderer('assignfeedback_onenote');
        ...
        $o .= $assignrenderer->render(new assign_header($assignment->get_instance(),
                                                        ...
                                                        get_string('uploadzipsummary', 'assignfeedback_onenote')));
