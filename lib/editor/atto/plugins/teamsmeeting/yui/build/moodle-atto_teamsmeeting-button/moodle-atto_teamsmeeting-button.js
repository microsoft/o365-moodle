YUI.add('moodle-atto_teamsmeeting-button', function (Y, NAME) {
    ...
    TEMPLATE = '' +
            '<form class="atto_form">' +
                '<div class="meeting-app">' +
                    ...
                    '</label>' +
                    '<iframe id="meetingapp" src="{{appurl}}?url={{clientdomain}}&locale={{locale}}' +
                    '&msession={{msession}}&editor=atto&previewmode=options"></iframe>' +
                '</div>' +
                '<div class="mb-1">' +
                    '<label for="{{elementid}}_atto_teamsmeeting_urlentry">{{get_string "meetingurl" component}}</label>' +
                    '<input class="form-control fullwidth url {{CSSNAMES.URLINPUT}}" type="url" ' +
                    'id="{{elementid}}_atto_teamsmeeting_urlentry" size="32" disabled="disabled"/>' +
                '</div>' +
                '<div class="form-check">' +
                    '<input type="checkbox" class="form-check-input newwindow" id="{{elementid}}_{{CSSNAMES.NEWWINDOW}}"/>' +
                    '<label class="form-check-label" for="{{elementid}}_{{CSSNAMES.NEWWINDOW}}">' +
...
Y.namespace('M.atto_teamsmeeting').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    /**
     * Signed token to pass to the Meetings app and back to result.php, where it
     * authenticates the request in place of the (possibly dropped) session cookie.
     *
     * @param _msession
     * @type String
     * @private
    ...
    _meetingcheck: function() {
        var iframe = this._content.one('#meetingapp');
        // The contentDocument is only readable when the Meetings app is hosted on
        // the same origin as Moodle; for a cross-origin app it is null and the
        // meeting link is picked up via the ajax lookup instead.
        if (iframe && iframe.getDOMNode().contentDocument) {
            var url = iframe.getDOMNode().contentDocument.location;
    ...
    _setteamsmeeting: function(e) {
        ...
        if (value !== '') {
            ...
            // Only http(s) links may be inserted. Normalise protocol-relative and
            // scheme-less values to https, and drop anything carrying another
            // scheme (javascript:, data:, ...) so it can never become an unsafe href.
            if (/^\/\//.test(value)) {
                value = 'https:' + value;
