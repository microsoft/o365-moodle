...
function xmldb_local_o365_upgrade($oldversion) {
    ...
    if ($result && $oldversion < 2015111917.01) {
        set_config('sharepointcourseselect', 'off', 'local_o365');
        upgrade_plugin_savepoint($result, 2015111917.01, 'local', 'o365');
    }

    if ($oldversion < 2015111917.02) {
        ...
        upgrade_plugin_savepoint($result, 2015111917.02, 'local', 'o365');
    }

    if ($result && $oldversion < 2015111917.03) {
        if ($dbman->table_exists('local_o365_coursegroupdata')) {
        ...
        upgrade_plugin_savepoint($result, '2015111917.03', 'local', 'o365');
    }

    if ($result && $oldversion < 2015111918.01) {
    ...
    if ($result && $oldversion < 2015111923.05) {
        if ($dbman->table_exists('local_o365_objects')) {
            $table = new xmldb_table('local_o365_objects');
        ...
        upgrade_plugin_savepoint($result, '2015111923.05', 'local', 'o365');
    }

    if ($result && $oldversion < 2015111923.06) {
        if ($dbman->table_exists('local_o365_objects')) {
            $table = new xmldb_table('local_o365_objects');
        ...
        upgrade_plugin_savepoint($result, '2015111923.06', 'local', 'o365');
    }
