...
function xmldb_local_o365_upgrade($oldversion) {
    ...
    if ($result && $oldversion < 2015012718) {
        $usersync = get_config('local_o365', 'aadsync');
        if ($usersync === '1') {
            set_config('aadsync', 'create', 'local_o365');
        }
        upgrade_plugin_savepoint($result, '2015012718', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012719) {
        ...
        upgrade_plugin_savepoint($result, '2015012719', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012720) {
        ...
        upgrade_plugin_savepoint($result, '2015012720', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012725) {
        if ($dbman->table_exists('local_o365_aaduserdata')) {
            $now = time();
        ...
        upgrade_plugin_savepoint($result, '2015012725', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012727) {
    ...
    if ($result && $oldversion < 2015012731.01) {
    ...
    if ($result && $oldversion < 2015012731.02) {
        ...
        upgrade_plugin_savepoint($result, '2015012731.02', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012732.01) {
        ...
        upgrade_plugin_savepoint($result, '2015012732.01', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012732.03) {
        // Create new indexes.
        ...
        upgrade_plugin_savepoint($result, '2015012732.03', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012734) {
        $table = new xmldb_table('local_o365_appassign');
        ...
        upgrade_plugin_savepoint($result, '2015012734', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012736) {
        ...
        upgrade_plugin_savepoint($result, '2015012736', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012742.01) {
        $table = new xmldb_table('local_o365_appassign');
        $field = new xmldb_field('photoupdated', XMLDB_TYPE_INTEGER, '10', null, null, null, null, 'photoid');
        if ($dbman->field_exists($table, $field)) {
        ...
        upgrade_plugin_savepoint($result, '2015012742.01', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012744) {
        ...
        upgrade_plugin_savepoint($result, '2015012744', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012744.02) {
        $config = get_config('local_o365');
        if (!empty($config->creategroups)) {
            set_config('creategroups', 'onall', 'local_o365');
        }
        upgrade_plugin_savepoint($result, '2015012744.02', 'local', 'o365');
    }

    if ($oldversion < 2015012745.02) {
        ...
        upgrade_plugin_savepoint(true, 2015012745.02, 'local', 'o365');
    }

    if ($result && $oldversion < 2015012745.03) {
        // Drop index.
        $table = new xmldb_table('local_o365_token');
    ...
    if ($result && $oldversion < 2015012748.01) {
        set_config('sharepointcourseselect', 'off', 'local_o365');
        upgrade_plugin_savepoint($result, 2015012748.01, 'local', 'o365');
    }

    if ($oldversion < 2015012748.02) {
        ...
        upgrade_plugin_savepoint($result, 2015012748.02, 'local', 'o365');
    }

    if ($result && $oldversion < 2015012748.03) {
        if ($dbman->table_exists('local_o365_coursegroupdata')) {
        ...
        upgrade_plugin_savepoint($result, '2015012748.03', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012749.01) {
        ...
        upgrade_plugin_savepoint($result, '2015012749.01', 'local', 'o365');
    }

    if ($result && $oldversion < 2015012752.01) {
        ...
        upgrade_plugin_savepoint($result, '2015012752.01', 'local', 'o365');
    }
