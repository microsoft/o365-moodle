 ...
 */
class apponlytoken extends \local_o365\oauth2\token {
    /**
    ...
    protected static function get_stored_token($userid, $resource) {
        $tokens = get_config('local_o365', 'apponlytokens');
     ...
     * @param boolean $apponlyaccess True to set instance to app only.
     * @return \local_o365\oauth2\token|bool A constructed token for the new resource, or false if failure.
     */
    public static function instance($userid, $resource, \local_o365\oauth2\clientdata $clientdata, $httpclient, $apponlyaccess = true) {
        return parent::instance($userid, $resource, $clientdata, $httpclient, $apponlyaccess);
    }
    ...
    protected function update_stored_token($existingtoken, $newtoken) {
        ...
        set_config('apponlytokens', $tokens, 'local_o365');
        return true;
    }
