...
var local_o365_coursesync_all_set_feature = function(state) {
    ...
    /**
     * Print the header shared by the "Manage Team Connections" tool and its action pages (connect, update, update
     * cache): the standard Moodle header with the breadcrumb hidden, a plugin-wide heading, the local_o365 settings
     * tab navigation, and the page-specific subtitle.
     *
     * @param string $subtitle The page-specific subtitle to print below the tab navigation.
     */
    protected function print_teamconnections_header(string $subtitle): void {
        global $OUTPUT, $PAGE;
    ...
    public function mode_teamconnections() {
        ...
        $PAGE->requires->jquery();
        $PAGE->requires->css('/local/o365/lib/datatables/css/jquery.dataTables.min.css');
        $this->print_teamconnections_header($this->title);
    ...
    public function mode_teamconnections_connect() {
        ...
        if ($mform->is_cancelled()) {
            ...
            $this->print_teamconnections_header($subtitle);
