...
var local_o365_coursesync_all_set_feature = function(state) {
    if (confirm("' . get_string('acp_coursesynccustom_confirm_all_action', 'local_o365') . '")) {
        ...
        echo html_writer::empty_tag('hr');
        $coursesynccustomisesettingheader = new admin_setting_heading(
            'local_o365/course_sync_customize_header',
    ...
    public function mode_coursesynccustom_change() {
        ...
        if ($existingcoursesyncpercoursesetting != $controlpercoursesetting) {
            add_to_config_log(
                ...
                $existingcoursesyncpercoursesetting,
                $controlpercoursesetting,
                'local_o365'
            );
        }
    ...
    public function mode_teamconnections_update() {
        ...
        if ($mform->is_cancelled()) {
            ...
            if (
                $courseusersyncdirection == COURSE_USER_SYNC_DIRECTION_MOODLE_TO_TEAMS ||
    ...
    /**
     * Print the header shared by the "Manage User Connections" tool and its action pages (resync, manual match,
     * unmatch, disconnect): the standard Moodle header with the breadcrumb hidden, a plugin-wide heading, the
     * local_o365 settings tab navigation, and the page-specific subtitle.
     *
     * @param string $subtitle The page-specific subtitle to print below the tab navigation.
     */
    protected function print_userconnections_header(string $subtitle): void {
        global $OUTPUT, $PAGE;
        ...
        echo $OUTPUT->header();
        // Hide the breadcrumb: the tab navigation below is the primary way to move between local_o365 settings pages.
        echo html_writer::tag('style', '#page-navbar { display: none; }');

        $pageheading = get_string('settings_pageheading', 'local_o365', get_string('pluginname', 'local_o365'));
        echo html_writer::tag('h2', $pageheading);

        // Tab navigation between the local_o365 settings pages.
        echo local_o365_get_settings_nav_html('local_o365_advanced');
        echo html_writer::tag('h4', $subtitle);
    }
    ...
    public function mode_userconnections() {
        ...
        $PAGE->requires->jquery();
        $PAGE->requires->css('/local/o365/lib/datatables/css/jquery.dataTables.min.css');
        $this->print_userconnections_header($this->title);
    ...
    public function mode_userconnections_resync(): bool {
        ...
        $this->print_userconnections_header($this->title);
        echo $OUTPUT->heading(get_string('acp_userconnections_table_resync', 'local_o365'), 5);
