...
function xmldb_auth_oidc_upgrade($oldversion) {
    ...
    if ($oldversion < 2025040830.02) {
        // Retry adding the unique constraint: the previous (2025040825.01) attempt used a helper
        ...
        utils::add_token_unique_constraint();
        upgrade_plugin_savepoint(true, 2025040830.02, 'auth', 'oidc');
    }

    if ($oldversion < 2025040830.04) {
        ...
        // Oidc savepoint reached.
        upgrade_plugin_savepoint(true, 2025040830.04, 'auth', 'oidc');
    }
    ...
    if ($oldversion < 2025040830.08) {
        \auth_oidc\utils::migrate_removed_icon_choices();
        upgrade_plugin_savepoint(true, 2025040830.08, 'auth', 'oidc');
    }

    if ($oldversion < 2025040835.02) {
        \auth_oidc\utils::repair_failed_icon_migration();
        upgrade_plugin_savepoint(true, 2025040835.02, 'auth', 'oidc');
    }
