...
if (!empty($action)) {
	...
	} elseif ($action === 'disconnect' && $oidcconnected === true) {
	...
	echo \html_writer::tag('h4', get_string('ucp_status', 'auth_oidc', $opname), ['style' => 'display: inline-block; margin-right: 0.5rem;']);
	if ($oidcconnected === true) {
		echo \html_writer::tag('h4', get_string('ucp_status_enabled', 'auth_oidc'), ['class' => 'notifysuccess', 'style' => 'display: inline-block']);
	} else {
		echo \html_writer::tag('h4', get_string('ucp_status_disabled', 'auth_oidc'), ['class' => 'notifyproblem', 'style' => 'display: inline-block']);
	}
	...
	if ($oidcconnected === true) {
		if (is_enabled_auth('manual') === true) {
			...
			$linkhtml = \html_writer::link($connectlinkuri, get_string('ucp_connected_disconnect', 'auth_oidc', $opname));
